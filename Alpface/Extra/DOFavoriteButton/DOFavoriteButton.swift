...
open class DOFavoriteButton: UIButton {
    ...
    fileprivate lazy var imageShape: CAShapeLayer = {
    ...
    fileprivate lazy var circleShape: CAShapeLayer = {
    ...
    fileprivate lazy var lines: [CAShapeLayer] = [CAShapeLayer]()
    ...
    /// 由于createLayers时，可能self的frame还没有获取到，此时则在layoutSubviews中初始化
    fileprivate var isCreateLayersInLayoutSubviews: Bool = false
    ...
    open override func layoutSubviews() {
        ...
        if isCreateLayersInLayoutSubviews == true {
            if frame.equalTo(.zero) == false {
                createLayers(image: image)
                isCreateLayersInLayoutSubviews = false
            }
        }
    }
    ...
    fileprivate func createLayers(image: UIImage!) {
        ...
        if frame.equalTo(.zero) == true {
            isCreateLayersInLayoutSubviews = true
            return
        }
