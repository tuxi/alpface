...
#define xy_DEFAULT_TO_VIEW(isWindow) (isWindow ? (UIView *)[UIApplication sharedApplication].delegate.window : [UIViewController xy_topViewController].view)
...
+ (void)xy_showActivityToView:(UIView *)view {
    [self xy_showActivityDelayTime:0 toView:view];
}
...
+ (void)xy_showActivityDelayTime:(CGFloat)delayTime
                          toView:(UIView *)view {
    [self xy_showActivityMessage:nil delayTime:delayTime toView:view];
}

+ (void)xy_showActivityMessage:(NSString*)message  {
    [self xy_showActivityMessage:message isWindow:YES delayTime:0];
}
...
+ (void)xy_showActivityMessage:(NSString*)message
    ...
    [self xy_showActivityMessage:message
                       delayTime:delayTime
                          toView:xy_DEFAULT_TO_VIEW(isWindow)];
}
...
+ (void)xy_showMaxTimeMessage:(NSString *)message {
    [self xy_showMessage:message delayTime:NSIntegerMax isWindow:NO];
}

+ (void)xy_showMessage:(NSString *)message {
    [self xy_showMessage:message delayTime:BBToastDefaultDuration isWindow:NO];
}
...
+ (MBProgressHUD *)xy_hudWithMessage:(NSString *)message
                            isWindow:(BOOL)isWindow {
    MBProgressHUD *hud = [self xy_hudWithMessage:message toView:xy_DEFAULT_TO_VIEW(isWindow) offset:CGPointZero];
    return hud;
}
...
- (void)xy_showProgressMessage:(NSString *)message
                actionCallBack:(XYHUDActionCallBack)callBack {
    [MBProgressHUD xy_showProgressMessage:message actionCallBack:callBack];
}
